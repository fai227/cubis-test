...
void rini_save_config(rini_config config, const char *file_name)
{
    ...
    if (rini_file != NULL)
    {
        for (unsigned int i = 0; i < config.count; i++)
        {
            ...
            {
                if (config.values[i].desc[0] != '\0')
                    fprintf(rini_file, "%c %s\n", RINI_LINE_COMMENT_DELIMITER, config.values[i].desc);
                else
            ...
            else
            {
                ...
                if (((value == 0) && (strlen(config.values[i].text) == 1) && (config.values[i].text[0] == '0')) || (value != 0))
                    valueIsInt = true;
...
#define RINI_MAX_TEXT_FILE_SIZE 4096
    ...
    int requiredSize = 0;
    for (unsigned int i = 0; i < config.count; i++)
        requiredSize += ((int)strlen(config.values[i].key) + (int)strlen(config.values[i].text) + (int)strlen(config.values[i].desc) + 64);
    if (requiredSize > RINI_MAX_TEXT_FILE_SIZE)
        RINI_LOG("WARNING: Required config.ini size is bigger than max supported memory size, increase RINI_MAX_TEXT_FILE_SIZE\n");
    ...
    if (key != NULL)
    {
        ...
        for (unsigned int i = 0; i < config->count; i++)
        {
            if (strcmp(key, config->values[i].key) == 0) // Key found
            {
                ...
                if (desc != NULL)
                    memcpy(config->values[i].desc, desc, strlen(desc));
                result = 0;
                break;
            }
        }
    ...
    if (result == -1)
    {
        if (config->count < config->capacity)
        {
            ...
            if ((key == NULL) && (text[0] == RINI_LINE_COMMENT_DELIMITER))
            {
                ...
                if (desc != NULL)
                    for (int i = 0; (i < RINI_MAX_DESC_SIZE) && (desc[i] != '\0'); i++)
                        config->values[config->count].desc[i] = desc[i];
                else
                    config->values[config->count].desc[0] = '\0';
            }
            else
            {
                // NOTE: We do a manual copy to avoid possible overflows on input data
                for (int i = 0; (i < RINI_MAX_KEY_SIZE) && (key[i] != '\0'); i++)
                    config->values[config->count].key[i] = key[i];
                for (int i = 0; (i < RINI_MAX_TEXT_SIZE) && (text[i] != '\0'); i++)
                    config->values[config->count].text[i] = text[i];
                for (int i = 0; (i < RINI_MAX_DESC_SIZE) && (desc[i] != '\0'); i++)
                    config->values[config->count].desc[i] = desc[i];
            }
...
{
    int len = 0;
    while ((buffer[len] != '\0') && (buffer[len] != ' ') && (buffer[len] != RINI_VALUE_DELIMITER))
        len++; // Skip keyentifier
...
static int rini_read_config_value_text(const char *buffer, char *text, char *desc)
{
    ...
    while ((buffer_ptr[0] != '\0') && (buffer_ptr[0] != ' '))
        buffer_ptr++; // Skip keyentifier

    while ((buffer_ptr[0] != '\0') && (buffer_ptr[0] == ' '))
        buffer_ptr++; // Skip line spaces before text value or delimiter
    ...
    if (buffer_ptr[0] == RINI_VALUE_DELIMITER)
    {
        ...
        while ((buffer_ptr[0] != '\0') && (buffer_ptr[0] == ' '))
            buffer_ptr++; // Skip line spaces before text value
    }
#endif
    ...
    for (; desc_pos < len; desc_pos++)
    {
        if (buffer_ptr[desc_pos] == RINI_DESCRIPTION_DELIMITER)
        {
            ...
            while (buffer_ptr[value_len] == ' ')
                value_len--;

            value_len++;

            desc_pos++; // Skip delimiter and following spaces
            while (buffer_ptr[desc_pos] == ' ')
                desc_pos++;
            break;
        }
    }
    ...
    if (buffer_ptr[0] == RINI_VALUE_QUOTATION_MARKS)
    {
        ...
        if (buffer_ptr[value_len - 1] == RINI_VALUE_QUOTATION_MARKS)
        {
            value_len--;
        }
    }
